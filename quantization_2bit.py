...
class Custom2BitQuantization(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, is_activation=False):
        ctx.save_for_backward(input)
        ctx.is_activation = is_activation
        if is_activation:
            # Activation: 0, 0.33, 0.67, 1로 양자화
            output = torch.clamp(input, 0, 1)
            output = torch.round(output * 3) / 3
        else:
            # 가중치: -1, -0.33, 0.33, 1로 양자화
            output = torch.clamp(input, -1, 1)
            output = torch.round(output * 1.5) / 1.5
        return output
...
class QuantizedConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        self.quantize = Custom2BitQuantization.apply
...
class QuantizedResNet18FeatureExtractor_2bit(nn.Module):
    def __init__(self, original_model):
        ...
        self.fc = original_model.fc  # fc layer도 복사
...
def feature_to_distribution(features):
	...
	features_pos = F.relu(features)  # Ensure non-negativity
