...
def save_checkpoint(
    state, is_SA_best, save_path, pruning, filename="checkpoint.pth.tar", args=None
):
    filepath = os.path.join(save_path, str(pruning) + "checkpoint"+str(args.dataset)+".pth.tar")
...
def load_checkpoint(device, save_path, pruning, filename="checkpoint.pth.tar", args=None):
    filepath = os.path.join(save_path, str(pruning) + "checkpoint"+str(args.dataset)+".pth.tar")
...
def setup_model_dataset(args):
    if args.dataset == "cifar10":
        ...
        if args.imagenet_arch:
            model = model_dict[args.arch](num_classes=classes, imagenet=True, pretrained=True)
        else:
        ...
        if args.class_to_replace is None:
            loaders = prepare_data(dataset="imagenet", batch_size=args.batch_size,data_path=args.data)
